...
import { getCloudStorageList, updateMatch302 } from '@/services/film-fusion';
...
> = {
  source_path: { key: 'basic', tab: '基本信息', label: '源路径' },
  cloud_storage_id: { key: 'basic', tab: '基本信息', label: '云存储' },
  target_path: { key: 'basic', tab: '基本信息', label: '目标路径' },
  balance_enabled: { key: 'balance', tab: '负载均衡', label: '启用负载均衡' },
  balance_strategy: { key: 'balance', tab: '负载均衡', label: '选择策略' },
  balance_limit_mode: {
    key: 'balance',
    tab: '负载均衡',
    label: '并发限制模式',
  },
  source_weight: { key: 'balance', tab: '负载均衡', label: '源账号权重' },
  pool_members: { key: 'balance', tab: '负载均衡', label: '子账号池' },
  cleanup_enabled: { key: 'cleanup', tab: '缓存清理', label: '自动清理' },
  retention_hours: { key: 'cleanup', tab: '缓存清理', label: '播放后保留小时' },
  cleanup_mode: { key: 'cleanup', tab: '缓存清理', label: '清理模式' },
    ...
    <>
      ...
      <ModalForm<API.UpdateMatch302Params>
        ...
        initialValues={{
          ...
          balance_enabled: record.balance_enabled,
          balance_strategy: record.balance_strategy || 'sticky_least_active',
          balance_limit_mode: record.balance_limit_mode || 'loose',
          source_weight: record.source_weight || 1,
          cleanup_enabled: record.cleanup_enabled ?? true,
          retention_hours: record.retention_hours || 72,
          cleanup_mode: record.cleanup_mode || 'recycle',
          cleanup_interval_min: record.cleanup_interval_min || 30,
          min_keep_ready: record.min_keep_ready || 0,
          pool_members: record.pool_members || [],
        }}
